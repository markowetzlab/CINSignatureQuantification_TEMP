# Quantify pan-cancer signature activities on any copy number profiles 
...
## Data
BASE=dirname(dirname(this.path()))
OUT=file.path(BASE, "output")
OUTFILEADDON="_PCAWG_WGS_downSNP6"

# CN profiles and their correction
DAT=file.path(BASE, "example/478_PCAWG_WGS_downSNP6_withNormals_penalty70.rds")
WIGGLE=0.1
IGNOREDELS=FALSE
CORES=6
...
write.table(dfAllSegs, file.path(OUT, paste0("0_absolute_CN", OUTFILEADDON, ".txt")), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
...
saveRDS(object = Hraw, file = file.path(OUT, paste0("4_Activities_to_TCGA_Signatures_raw", OUTFILEADDON, ".rds")))

H = t( apply(Hraw, 2, function(x) x/sum(x)) )
write.table(x = H, file = file.path(OUT, paste0("4_Activities_to_TCGA_Signatures", OUTFILEADDON, ".txt")), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")
saveRDS(object = H, file = file.path(OUT, paste0("4_Activities_to_TCGA_Signatures", OUTFILEADDON, ".rds")))
