...
plotSegments <- function(object=NULL,sample=NULL,cn.max=15){
    ...
    chrom.len$x.max <- stats::aggregate(segTab$end,by = list(segTab$chromosome),FUN = max)$x
    chrom.len$x.min <- stats::aggregate(segTab$start,by = list(segTab$chromosome),FUN = min)$x
    chrom.len$Group.1 <- factor(chrom.len$Group.1,levels = stringr::str_sort(unique(chrom.len$Group.1),numeric = T))
    chrom.len <- chrom.len[order(chrom.len$Group.1),]

    segTab$startf <- getcoordinates(chr = segTab$chromosome,pos = segTab$start,chrom.len = chrom.len)
    segTab$endf <- getcoordinates(chr = segTab$chromosome,pos = segTab$end,chrom.len = chrom.len)
    chrom.len$flatm <- getcoordinates(chr = chrom.len$Group.1,pos = chrom.len$x.min,chrom.len = chrom.len)
    chrom.len$flats <- getcoordinates(chr = chrom.len$Group.1,pos = chrom.len$x.max/2,chrom.len = chrom.len)
    chrom.len$flate <- getcoordinates(chr = chrom.len$Group.1,pos = chrom.len$x.max,chrom.len = chrom.len)

    title <- samp.name
    sub.title <- paste0("ploidy: ",ob.pl," | segments: ",seg.n)
    rect.col <- ifelse(seq_along(chrom.len$Group.1) %% 2 == 0,"white","grey95")

    graphics::par(mar=c(5, 4, 4, 4) + 0.2)
    graphics::plot(NA,
         xlab="chromosome",
    ...
    graphics::abline(h = seq.int(0,cn.max-1,1),lty="dashed",col="gray50")
    graphics::segments(x0 = segTab$startf,y0 = segTab$segVal,x1 = segTab$endf,y1 = segTab$segVal,lwd=3,col="blue")
}
